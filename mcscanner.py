...
def print_time(threadName):
    for z in split[int(threadName)]:
        ...
        try:
            ...
            print("Found server: " + ip + " " + status.version.name + " " + str(status.players.online) + " Players online, Names of players: " + {(query.players.names)})
                    ...
                    if ip not in f.read():
                        with open(publicserverlist) as g:
                            if ip not in g.read():
                                ...
                                text_file.write(ip + " " + status.version.name.replace(" ", "_") + " " + str(status.players.online) + " Players online, Names of players: " + {(query.players.names)})
